...
class xai_rnn(object):
    ...
    def xai_feature(self, samp_num, option= 'None'):
        ...
        label_sampled = label_sampled.reshape(label_sampled.shape[0], 1)#(501,1)
        #print "label_sampled",label_sampled
        print "label_sampled_shape",label_sampled.shape
...
if __name__ == "__main__":
    ...
    print "x_test_shape",x_test.shape#x_test.shape (6003, 200)
    print "y_test_shape",y_test.shape#y_test.shape (6003, 200, 2)
    # extract all the function starts for explanation
    print "nozero",np.nonzero(y)#nozero (array([   8,   14,   14, ..., 5977, 5988, 5988]), array([ 99,  68,  83, ..., 144, 120, 164]))
    idx = np.nonzero(y)[0]
    ...
    for i in xrange(len(x_test)):
        ...
        if i in idx:
            ...
            print "x_test_d_shape",x_test_d.shape#(1,200)
            for j in xrange(len(idx_Row)):
                xai_test = xai_rnn(model, x_test_d, binary_func_start[j], idx_Row[j])
                print "pred",xai_test.pred#[[1.7976262e-09 1.0000000e+00]]
                print "xai_test_pred",xai_test.pred[0,1]#1.0
